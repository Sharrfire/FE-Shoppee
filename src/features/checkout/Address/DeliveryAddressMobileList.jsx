      ...
      <div>
        <ul className='checkout__address-moblie-list'>
          {addressList.map((address) => (
            <li key={address.id} className='checkout__address-mobile-item' onClick={() => handleChange(address)}>
              <div className='checkout__address-mobile-name'>
                <span className='checkout__address-mobile-name1'>{address.name}</span>
                {address.status === true && <span className='checkout__address-mobile-default'>[Mặc định]</span>}
              </div>
